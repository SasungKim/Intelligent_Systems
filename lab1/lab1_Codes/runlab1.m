...
% FOR x1 data
%for i=1:length(x1)
%   [posteriors_x,g_x,g_x2]=lab1(x1(i),trainingSet);
%   if g_x2==1
%       disp('SAMPLE IS FROM CLASS w_1')   
%       fprintf('\n')
%   else
%       disp('SAMPLE IS FROM CLASS w_2')
%       fprintf('\n')
%   end
%end
% 
 % FOR x2 (sepal width) data
 for i=1:length(x1)
    [posteriors_x,g_x,g_x2]=lab1_x2(x1(i),trainingSet);
    if g_x2==1
        disp('SAMPLE IS FROM CLASS w_1')   
        fprintf('\n')
    else
