...
%              Sepal Length
function [posteriors_x,g_x,g_x2]=lab1(x,Training_Data)
...
f=D(:,1);  % feature samples (all rows, column 1: sepal length)
...
m11 =  mean(D(1:class1_count,1)) % mean of the class conditional density p(x/w1)
std11 = std(D(1:class1_count,1)) % Standard deviation of the class conditional density p(x/w1)

m12 = mean(D(class1_count+1:total_count,1)) % mean of the class conditional density p(x/w2)
std12= std(D(class1_count+1:total_count,1)) % Standard deviation of the class conditional density p(x/w2)
